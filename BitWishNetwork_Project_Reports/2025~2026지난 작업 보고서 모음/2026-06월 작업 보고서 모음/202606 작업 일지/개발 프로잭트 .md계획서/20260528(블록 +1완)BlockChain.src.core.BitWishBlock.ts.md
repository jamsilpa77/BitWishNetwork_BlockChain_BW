...
import { BITWISH_BLOCK_CONFIG, BITWISH_TRANSACTION_CONFIG } from '../config/BitWishConfig';
...
export class BitWishBlock {
  public header: BitWishBlockHeader;
  public transactions: BitWishTransaction[];
  ...
  static createGenesisBlock(): BitWishBlock {
    const genesisHeader: BitWishBlockHeader = {
      version: BITWISH_BLOCK_CONFIG.VERSION,
      previousHash: BITWISH_BLOCK_CONFIG.GENESIS_HASH,
      merkleRoot: BITWISH_BLOCK_CONFIG.MERKLE_ROOT_EMPTY,
      timestamp: Date.now(),
      difficulty: 1,
      nonce: 0,
  ...
  calculateMerkleRoot(): string {
    if (this.transactions.length === 0) {
      return BITWISH_BLOCK_CONFIG.MERKLE_ROOT_EMPTY;
    }

    // [긴급 수복] 트랜잭션 해시 미계산 방어 - undefined 크래시 원천 차단
    for (const tx of this.transactions) {
      if (!tx.hash && typeof tx.calculateHash === 'function') {
        tx.hash = tx.calculateHash();
      }
    }

    if (this.transactions.length === 1) {
      return createHash('sha256').update(this.transactions[0].hash || '').digest('hex');
    }
  ...
  sign(privateKey: string): string {
    ...
    return createHmac('sha256', privateKey).update(blockData).digest('hex');
  }
  ...
  verifySignature(publicKey: string, signature: string): boolean {
    try {
      ...
      const blockData = this.calculateHash();
      const expectedSignature = createHmac('sha256', publicKey).update(blockData).digest('hex');
   ...
   * 블록 마이닝 (PoW)
   */
  mine(targetDifficulty: number): { success: boolean; hash?: string; nonce?: number } {
