 ...
 */
const AttendanceCalendar = memo(({ onClose }: { onClose?: () => void }) => {
  // 절대 준수사항: 전역 변수 사용 금지
  ...
  const [isModalOpen, setIsModalOpen] = useState(true); // 수정: 기본값을 true로 변경하여 열림 상태 유지
  ...
  const startRealTimeSync = (): (() => void) => {
    try {
      const interval = setInterval(() => {
        checkAttendanceStatus();
        updateBonusRate();
        // 수정: 1초마다 불필요하게 서버를 호출하여 브라우저 클릭을 먹통으로 만드는 코드 제거
        // loadAttendanceRecords(); 
      }, 1000); // 1초마다 내부 가벼운 상태값만 업데이트
  ...
  const handleAttendanceCheck = async (date: Date): Promise<void> => {
    try {
      // 수정: 출석 보너스를 등록할 때 누락되었던 지갑 주소를 정상적으로 전달합니다.
      const result: { success: boolean; message: string } = await attendanceBonusService.applyAttendanceBonus(WALLET_ADDRESS);
      if (result.success) {
  ...
  const handleCloseModal = (): void => {
    try {
      ...
      if (onClose) onClose(); // 수정: 부모 컴포넌트에 닫기 요청 전달
    } catch (error) {
  ...
  if (!isModalOpen) return null; // 추가: 닫기 상태가 되면 달력 전체 출력을 종료하여 모달을 닫습니다.
