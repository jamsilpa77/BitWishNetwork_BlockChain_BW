...
const MyWalletModal: React.FC<MyWalletModalProps> = ({
    ...
    const blockTxDict: Record<string, Record<string, string>> = {
        ...
        copied: { ko: '해시가 복사되었습니다!', en: 'Hash copied!', ja: 'ハッシュがコピーされました！', zh: '哈希已复制！' },
        minting: { ko: '채굴 발행', en: 'Minting', ja: 'マイニング発行', zh: '挖矿铸造' },
    ...
    useEffect(() => {
        ...
        // (닫힐 때 미리 리셋해두어야 다음 열릴 때 이전 위치에서 날아오는 잔상이 없음)
        const width = 800; // Modal width
        const x = (window.innerWidth - width) / 2;
        const y = 300; // 지정 좌표(텍스트 살리고 버튼 덮음) 적용
        setPosition({ x, y });
    }, [isOpen]);
    ...
    useEffect(() => {
        ...
        const unsubscribe = realTimeSyncService.subscribe((data) => {
            if (data && data.currentIssued >= 0) {
                ...
                setWalletData(prev => ({
                    ...prev,
                    balance: new Decimal(data.currentIssued),           // 누적 채굴량 실시간 동기화
                    referralBonus: new Decimal(data.referralBonusStorage) // 2% 추천 보너스 실시간 동기화
                }));
            }
        });
    ...
    return (
        <div
            ...
            style={{
                pointerEvents: 'none', // 배경 클릭 통과 허용
                backgroundColor: 'transparent', // 배경 딤드 제거
        ...
        >
            <div
                ...
                style={{
                    ...
                    left: `${position.x}px`,
                    top: `${position.y}px`,
                    pointerEvents: 'auto', // 모달 내부 클릭은 허용
                    cursor: isDragging ? 'grabbing' : 'default',
                    zIndex: isActive ? 10100 : 10002, // 포커스 시 격상
                    ...
                    <div className="header-address-row">
                        ...
                        <div style={{ marginLeft: 'auto', display: 'flex', gap: '10px' }}>
                            ...
                            <button
                                ...
                                onClick={() => {
                                    if (onOpenMining) {
                                        onOpenMining(walletAddress); // [Fix] Pass walletAddress without closing wallet
                                    } else {
