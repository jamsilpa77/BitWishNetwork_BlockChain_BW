# [최종확정] BitWish 시스템 정상화 통합 실행 지침서
...
*   **핵심 데이터:**
    ...
    2.  **Current Issued:** 현재 발행량 (1초마 상승 ▲)
    3.  **Remaining Supply:** 잔여 발행량 (1초마다 감소 ▼)
    4.  **Issuance Rate:** 발행률 (%)
    5.  **Generated Blocks:** 생성된 블록 수

### Step 4. [보안] `src/services/BlockchainService/WalletService.ts`
...
### Step 4. 비밀번호 설정 로직 수정 (WalletService.ts)
```typescript
