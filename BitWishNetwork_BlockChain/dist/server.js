...
exports.BitWishNetworkServer = void 0;
const express_1 = __importDefault(require("express"));
const http_1 = require("http");
const socket_io_1 = require("socket.io");
const cors_1 = __importDefault(require("cors"));
const mongodb_1 = require("mongodb");
const BitWishBlockchain_1 = require("./engine/BitWishBlockchain");
const BitWishDiscovery_1 = require("./p2p/BitWishDiscovery");
const BitWishPoW_1 = require("./consensus/BitWishPoW");
const BitWishConfig_1 = require("./config/BitWishConfig");
const storage_1 = require("./server/storage"); // [신규] 영구 저장소 모듈 추가
// ====================================================================================
...
class BitWishNetworkServer {
    constructor() {
        ...
        this.io = new socket_io_1.Server(this.server, {
            cors: {
                origin: BitWishConfig_1.BITWISH_NETWORK_CONFIG.API_CONFIG.cors.origin,
                credentials: BitWishConfig_1.BITWISH_NETWORK_CONFIG.API_CONFIG.cors.credentials
            }
        });
        this.blockchain = new BitWishBlockchain_1.BitWishBlockchain();
        this.discovery = new BitWishDiscovery_1.BitWishDiscovery();
    ...
    setupRoutes() {
        ...
        this.app.get('/api/mining/status/:walletAddress', (req, res) => {
            try {
                ...
                let referrals = storage_1.PersistentStorage.loadFromFile('referrals.json') || { users: {} };
        ...
        this.app.post('/api/mining/sync', (req, res) => {
            try {
                ...
                if (!walletAddress)
                    return res.status(400).json({ success: false, message: '지갑 주소 누락' });
                let referrals = storage_1.PersistentStorage.loadFromFile('referrals.json') || { users: {} };
                if (!referrals.users)
                    referrals.users = {};
                ...
                storage_1.PersistentStorage.saveToFile('referrals.json', referrals);
        ...
        this.app.post('/api/storage/save', (req, res) => {
            try {
                ...
                // 저장 전 백업 자동 생성 (안전장치)
                storage_1.PersistentStorage.createBackup(filename);
                const success = storage_1.PersistentStorage.saveToFile(filename, data);
        ...
        this.app.get('/api/storage/load/:filename', (req, res) => {
            try {
                ...
                const data = storage_1.PersistentStorage.loadFromFile(filename);
                if (!data) {
            ...
            try {
                ...
                try {
                    if (this.mongoClient) {
                        ...
                        await db.collection('users').updateOne({ walletAddress: userData.walletAddress }, { $set: { ...userData, updatedAt: Date.now() } }, { upsert: true });
                    }
                }
                catch (mongoError) {
                ...
                const fileSaveSuccess = storage_1.PersistentStorage.saveToFile('referrals.json', referrals);
                return res.json({
                    success: true,
    ...
    async connectMongoDB() {
        try {
            this.mongoClient = new mongodb_1.MongoClient(BitWishConfig_1.BITWISH_NETWORK_CONFIG.DATABASE_CONFIG.connectionString);
            await this.mongoClient.connect();
            const db = this.mongoClient.db(BitWishConfig_1.BITWISH_NETWORK_CONFIG.DATABASE_CONFIG.databaseName);
            // 컬렉션 생성
    ...
    async start() {
        try {
            ...
            // 4. 서버 시작
            const port = BitWishConfig_1.BITWISH_NETWORK_CONFIG.API_CONFIG.port;
...
exports.BitWishNetworkServer = BitWishNetworkServer;
// ====================================================================================
// 서버 인스턴스 생성 및 시작
// ====================================================================================
