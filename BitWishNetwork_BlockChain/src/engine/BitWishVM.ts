...
export class BitWishVM {
    ...
    private storage: Map<string, string>; // 스마트 컨트랙트 상태 보관함 (영구 저장소)
        ...
        try {
            ...
            for (let pc = 0; pc < opcodes.length; pc++) {
                ...
                switch(op.instruction) {
                    case 'PUSH':
                        if (op.value) this.stack.push(op.value);
                        break;
                    case 'POP':
                        this.stack.pop();
                        break;
     ...
     */
    private consumeGas(op: Opcode) {
        let cost = 1; // 모든 연산의 기본 가스 비용
        
        switch(op.instruction) {
            case 'SSTORE': cost = 100; break; // DB 영구 저장은 가장 비싸게 설정
            case 'SLOAD': cost = 20; break;   // DB 읽기
            case 'SHA3': cost = 30; break;    // 해시 연산
            case 'CALL': cost = 50; break;    // 외부 호출
        }
     ...
     */
    private parseBytecode(code: string): Opcode[] {
     ...
     * 50자리 정밀도 보존 덧셈
     */
    private safeAdd(a: string, b: string): string {
        try {
