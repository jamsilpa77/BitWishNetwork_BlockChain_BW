...
router.get('/realtime', async (req, res) => {
    try {
        ...
        let totalBlocks = 0;
        try {
            const collections = await mongoose.connection.db?.listCollections({ name: 'blocks' }).toArray();
            if (collections && collections.length > 0) {
                totalBlocks = await mongoose.connection.db?.collection('blocks').countDocuments({}) || 0;
            }
        } catch (blockError) {
