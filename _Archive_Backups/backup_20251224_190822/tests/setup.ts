...
process.env.MONGODB_URI = 'mongodb://localhost:27017/BitWishNetwork_Test';
process.env.BITWISH_NETWORK_URL = 'http://localhost:3001';
...
beforeAll(async () => {
  // 절대 준수사항: 전역 변수 사용 금지
  console.log('🧪 BitWishNetwork 테스트 환경 초기화');
});

afterAll(async () => {
  // 절대 준수사항: 전역 변수 사용 금지
  console.log('🧹 BitWishNetwork 테스트 환경 정리');
});
