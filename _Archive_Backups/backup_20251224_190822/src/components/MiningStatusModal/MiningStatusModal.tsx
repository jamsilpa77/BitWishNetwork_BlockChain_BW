        ...
        const initializeData = async () => {
            try {
                ...
                if (userData) {
                    ...
                    // [FIX] 서버 데이터가 존재하면 화면 표시에 최우선 반영 (서비스 로직 무시)
                    if (realReferralCount > 0) {
                        console.log(`[MiningStatusModal] 서버 데이터 우선 적용: ${realReferralCount}명`);
                        referralStats = {
                            ...referralStats,
                            referredUsers: new Array(realReferralCount).fill('SERVER_SYNCED_USER'), // 길이를 맞추기 위한 더미 데이터
                            // totalBonusRate 속성이 있다면 업데이트, 없으면 무시
                        };
                    }

                    // 4. 최종 마이닝 상태 설정 (서버 + 로컬 하이브리드)
                    // 추천 보너스 비율 계산 (1명당 2%)
