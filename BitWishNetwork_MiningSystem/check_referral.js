...
async function checkReferralSystem() {
    try {
        ...
        users.forEach((user, idx) => {
            console.log(`[${idx+1}] 지갑: ${user.walletAddress}`);
        ...
        for (const record of bonusRecords) {
            if (record.referralList && record.referralList.length > 0) {
                ...
                record.referralList.forEach((ref, idx) => {
                    console.log(`    ${idx + 1}. 가입자: ${ref.childWalletAddress}`);
                    console.log(`       상태: [Status: ${ref.rewardStatus || 'NONE'}] [1BW_Paid: ${ref.is1BWMePaid || ref.is1BWUserPaid || 'N/A'}] [2%_Paid: ${ref.is2PercentMePaid || 'N/A'}]`);
                    console.log(`       가입일: ${new Date(ref.joinedAt).toLocaleString()}`);
                });
                console.log('');
            }
        }
        ...
        for (const user of users) {
            if (user.referrerCode) {
                const referrer = await User.findOne({ myReferralCode: (user.referrerCode || '').trim() });
                if (referrer) {
                    console.log(`${user.walletAddress} → (추천인: ${referrer.walletAddress}) [코드: ${user.referrerCode}]`);
                } else {
                    console.log(`❌ ${user.walletAddress} → 추천인 실종 [사용한 코드: '${user.referrerCode}']`);
                }
            }
        }
