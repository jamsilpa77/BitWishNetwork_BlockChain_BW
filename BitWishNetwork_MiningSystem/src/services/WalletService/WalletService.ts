...
export class WalletService {
     ...
     * 2차 비밀번호 설정
     */
    public setSecondPassword(address: string, password: string): boolean {
        ...
        // 해싱 (PBKDF2, 100000 iterations, 64 key length, sha512)
        const hash = crypto.pbkdf2Sync(password, salt, 100000, 64, 'sha512').toString('hex');

        // 저장 포맷: salt:hash
        // @ts-ignore - WalletData interface update required if strict
        wallet.secondPassword = `${salt}:${hash}`;
     ...
     * 2차 비밀번호 검증
     */
    public verifySecondPassword(address: string, password: string): boolean {
        const wallet = this.getStoredWallet();
        // @ts-ignore
        if (!wallet || wallet.address !== address || !wallet.secondPassword) {
            return false;
        }

        // @ts-ignore
        const [salt, storedHash] = wallet.secondPassword.split(':');
